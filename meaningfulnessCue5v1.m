% MeaningfulnessCue5v1.m - intact/scrambled intermixed, object random 0
...
for it = 1:totalN
    ...
    % remember(no-arrow) & forget(arrow) condition
    if dTrg(stmGrp, it) == 1
        if cTrg(stmGrp, it) <= 3
            % d1) obj1,obj2,obj3,obj4,obj5,obj6
            cntCase = cntCase + 1;
            wImageSet(dataFile, fJPG(idxJPG(cntCase, 1:3)));
            wBlanks(dataFile, 3);
    ...
    for tt = 1:6
        if cTrg(stmGrp, it) == tt
            ...
            fprintf(dataFile, 'd%dobj%d,', dTrg(stmGrp, it), tt);
        end
    end
    ...
    fprintf(dataFile, '%d,%d,%d,%s,,\n', setGrp-1, cTrg(stmGrp, it), dTrg(stmGrp, it), ctmp(1:end-4));
end
end
...
function wBlanks(fileID, count)
    fprintf(fileID, repmat('blank,', 1, count));
end
